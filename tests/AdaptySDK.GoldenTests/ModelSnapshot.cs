...
namespace AdaptySDK.GoldenTests
{
    ...
    /// private ones, which the SDK does serialize) and every public property, keys sorted.
    ///
    /// Snapshotting by reflection rather than by listing expectations by hand is what makes the
    /// fixtures cheap: 155 fields across the models are covered without enumerating them, and a
    /// field that silently stops being parsed shows up as a diff.
    ///
    /// Computed properties are invoked too — a throwing property is recorded as such, because
    /// that is also observable behaviour worth pinning.
    /// </summary>
