...
- **`Packages/com.adapty.unity-sdk/`** — The SDK package distributed to users (UPM layout):
  ...
  - `Runtime/Serialization/` — the Newtonsoft JSON layer: `AdaptyJson` (the single entry point), `AdaptyContractResolver`, `JsonRequire`, and `Converters/`. Every converter lives in that folder, one per file, named `AdaptyConverter<What>` after what it converts, and is registered in `AdaptyJson.Settings`.
