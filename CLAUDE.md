...
**The legacy onboarding API is maintained, not improved.** It has to keep working — it is still in the shipped public surface and apps still call it — so a bug in it gets fixed. What it does not get is breaking changes, and it is deliberately left out of sweeps that improve the live API: it kept its polymorphic converters through the converter audit, its `partial`-era shapes, and its mutable `IList` through the read-only collections work. When a sweep states a rule in the changelog, say plainly that the deprecated API is the exception rather than quietly implying it was covered — an inaccurate claim about it is a defect, touching it to make the claim true is not.
